...
async def _get_from_jikan(session: aiohttp.ClientSession, anime_name: str):
    """Source 1: Jikan (MyAnimeList wrapper) — picks the best match from top results."""
    try:
...
async def _get_from_anilist(session: aiohttp.ClientSession, anime_name: str):
    """Source 2: AniList (GraphQL) — fetches top 5 and picks best title match."""
    try:
        query = """
        query ($search: String) {
          Page(perPage: 5) {
        ...
        async with session.post(
            ...
            if resp.status == 200:
                ...
                logger.info(
                    ...
                    _best_title_score(anime_name, titles),
                    anime_name,
                )
...
async def _get_from_kitsu(session: aiohttp.ClientSession, anime_name: str):
    """Source 3: Kitsu API — fetches top 5 and picks best title match."""
    try:
