...
async def get_anime_info(anime_name: str):
    """
    Get anime text info from Jikan → AniList → Kitsu (first that works).
    Then replace the image with a Pinterest 16:9 fanart if available.
    Returns (caption, image_url).
    """
    async with aiohttp.ClientSession() as session:
        # Step 1: get caption + fallback poster from info APIs
        caption = None
        fallback_image = None
        for source_fn, source_name in [
            ...
            caption, fallback_image = await source_fn(session, anime_name)
            if caption and fallback_image:
                logger.info(f"Anime text info from {source_name} for '{anime_name}'")
                break
        ...
        # Step 2: try Wallhaven for a 16:9 fanart, fall back to info API poster.
        # Strip "Season 2" / "Part 3" suffixes before searching — Wallhaven tags
        # images by show title, not by season, so the base title gets more hits.
        fanart_url: str | None = None
        source = "info API poster"

        img_query = _image_search_name(anime_name)
        if img_query != anime_name:
            logger.info("Image search: using base title '%s' for '%s'", img_query, anime_name)

        wh_url = await _get_wallhaven_image(session, img_query)
        if wh_url:
            fanart_url = wh_url
            source = "Wallhaven"

        final_image = fanart_url or fallback_image
        logger.info(f"Image source for '{anime_name}': {source}")

        return caption, final_image
...
async def _download_via_animekai(
    ...
    try:
        ...
        for candidate in scored[:5]:
            try:
                episodes = await animekai.list_episodes(candidate.path, timeout=45.0)
            except Exception as e:
                logger.warning("AnimeKAI fallback: list_episodes failed for '%s': %s", candidate.title, e)
                continue
            if not episodes:
                continue
        ...
        if not chosen or not ep:
            logger.info("AnimeKAI fallback: episode %s not found for '%s'", episode, anime_name)
            return None
...
async def get_stream_links(anime_name: str, episode_number: str) -> str:
    ...
    try:
        ...
        chosen = None
        ep = None
        # Walk top candidates and pick the first one whose episode list contains
        # the requested episode. This is the real fix for "wrong series" matches.
        for candidate in scored[:5]:
            try:
                episodes = await animekai.list_episodes(candidate.path, timeout=45.0)
            except Exception as e:
                logger.warning(
                    ...
                    candidate.title, candidate.path, e,
                )
                continue
            if not episodes:
                logger.info("AnimeKAI: '%s' has no episodes", candidate.title)
                continue
            ...
            if match:
                ...
                ep = match
                logger.info(
                    "AnimeKAI: selected '%s' (%s) — has episode %s",
                    candidate.title, candidate.path, episode_number,
                )
                break
            else:
                logger.info(
                    "AnimeKAI: '%s' does not contain ep %s (has %d eps), trying next",
                    candidate.title, episode_number, len(episodes),
                )

        if not chosen or not ep:
            logger.info(
                "AnimeKAI: no candidate contained episode %s for '%s'",
