...
set_var() {
    _CURL_CMD="$(command -v curl)" || command_not_found "curl"
    _JQ="$(command -v jq)" || command_not_found "jq"
    # Replaced fzf with head for non-interactive mode
    _FZF="head -n 1" 
...
download_segments() {
    ...
    # We must export the function too
...
download_episode() {
    ...
    if [[ -z ${_LIST_LINK_ONLY:-} ]]; then
        ...
        if [[ ${_PARALLEL_JOBS:-} -gt 1 ]]; then
            ...
            # Updated ffmpeg to use User-Agent
            "$_FFMPEG" $extpicky -user_agent "$_USER_AGENT" -headers "Referer: $_REFERER_URL" -i "$pl" -c copy $erropt -y "$v"
        fi
    else
        echo "$pl"
    fi
...
main() {
    ...
    if [[ -n "${_INPUT_ANIME_NAME:-}" ]]; then
        # Modified to use head -n 1 instead of fzf for non-interactive mode
